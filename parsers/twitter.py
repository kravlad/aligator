...
async def send_tweets():
    auth = tweepy.OAuthHandler(sets['twitter']['consumer_key'], sets['twitter']['consumer_secret'])
    auth.set_access_token(sets['twitter']['access_token'], sets['twitter']['access_secret'])
    api = tweepy.API(auth)
    ...
    for user_id in sources:
        ...
        tweets = api.user_timeline(screen_name=user_id, since_id=bookmarks[user_id])
        if tweets:
            bookmarks[user_id] = tweets[0].id
            i = 1
            for tweet in reversed(tweets):
                if i <= 13:
                    msg = '{msg}\n🔹{n}. {tweet} | <a href="{lnk}{twid}">tweet</a>\n'.format(msg=msg, tweet=tweet.text, n=i, lnk=link, twid=tweet.id)
                    i += 1
                else:
